...
def game_over(score):
    ...
    now = datetime.now()
    dtnow = now.strftime("%d/%m/%Y %H:%M:%S")   # getting accurate date and time
    ...
    if fileEmpty == True:       # if the file was empty then write down current score and date
        ...
        hDate = fileData[1]     # getting the date separately
        file.close
        
        file = open("highest-score.txt", "w")
        if score >= int(hScore):
            hScore = str(score)
            file.write(hScore + "  -  " + dtnow) # writing data back to the file (if [1])
        else:
            file.write(hScore + "  -  " + hDate) # writing data back to the file (if [2])
        file.close()
